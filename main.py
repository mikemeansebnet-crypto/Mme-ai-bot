            ...
            for i, media_url in enumerate(media_urls_raw):
                try:
                    ...
                    if img_bytes[:8] == b'\x89PNG\r\n\x1a\n':
                        ...
                        img_type = "image/jpeg"
                    photo_urls.append(img_bytes)
                    detected_types.append(img_type)
                    print(f"PHOTO DOWNLOADED | {i+1} of {len(media_urls_raw)} | {len(img_bytes)} bytes | type={img_type}")
                except Exception as e:
...
IMPORTANT: Base the estimate ONLY on what the contractor describes in their notes. If the contractor says it is a small repair, price it as a small repair. Do not assume the worst case scenario from photos alone. The contractor's description of scope overrides what you see in photos.
...
Only set has_schedule to true if there is a specific date or time mentioned."""
            ...
            if _pm_match:
                ...
                if _pm_data.get("has_property_details") and _pm_data.get("address"):
                    ...
                    if _pm_save_resp.status_code >= 300:
                        print(f"PM AUTO-SAVE FAILED | {_pm_save_resp.status_code} | {_pm_save_resp.text}")
                    else:
                ...
                if _pm_data.get("has_schedule") and _pm_data.get("requested_date"):
                    ...
                    if appt_iso:
                        ...
                        if _pm_book_resp.status_code >= 300:
                            print(f"PM AUTO-BOOK FAILED | {_pm_book_resp.status_code} | {_pm_book_resp.text}")
                        else:
        ...
        if customer_phone:
            try:
                create_estimate_approval(
                    ...
                    quote_low=round(subtotal * 0.9, 2),
                    quote_high=round(subtotal * 1.1, 2),
                    materials=materials,
                    notes=notes,
                    twilio_number=twilio_number,
                    pdf_url=pdf_url or "",
...
def dashboard_govbid_generate_pdf():
    ...
    try:
        print("GOVBID PDF | Starting PDF generation")
        import json as _json
        from reportlab.pdfgen import canvas
        from reportlab.lib.pagesizes import letter
        from reportlab.lib import colors
        ...
        for req in reqs:
            req = req.strip()
            if req:
                check_data.append(["o", Paragraph(req, small_style), "Pending"])
        ...
        story.append(Paragraph("CERTIFICATION AND SIGNATURE", heading_style))
        story.append(Paragraph(
            "The undersigned hereby offers and agrees to furnish the services described "
            "in this solicitation at the prices stated above, subject to all terms and "
        ...
        sig_data = [
            ...
            ["", "", ""],
            ["Business Name:", business_name, ""],
            ["Phone:", notify_sms, "Email: " + notify_email],
        ]
        sig_table = Table(sig_data, colWidths=[1.5*inch, 3.0*inch, 2.25*inch])
        ...
        story.append(Paragraph(
            f"Generated by CrewCachePro | {business_name} | {date.today().strftime('%B %d, %Y')}",
            small_style
        ))
        ...
        import cloudinary.uploader
        pdf_temp_path = f"/tmp/bid_{solicitation_number}_{int(datetime.now().timestamp())}.pdf"
        with open(pdf_temp_path, "wb") as f:
            f.write(pdf_bytes)
        ...
        try:
            result = cloudinary.uploader.upload(
                pdf_bytes,
                public_id=f"contractoros/bids/{solicitation_number}",
                resource_type="raw",
                format="pdf",
                overwrite=True,
            )
            raw_url = result.get("secure_url", "")
            pdf_url = raw_url.replace("/upload/", "/upload/fl_attachment/")
            print(f"GOVBID PDF | Uploaded to Cloudinary | {pdf_url}")
        except Exception as cloud_err:
            print(f"GOVBID PDF | Cloudinary error (non-fatal) | {cloud_err}")
        ...
        if record_id:
            try:
                ...
                requests.patch(
                    ...
                    headers={"Authorization": f"Bearer {AIRTABLE_TOKEN}", "Content-Type": "application/json"},
                    json={"fields": {
                        "fldqnOCsiQ2lGibcY": pdf_url or "",
                        "fld5fIztR19bRc0UU": total,
                        "fldHOIcdY5lcG13Sv": "Draft",
                    }}
                )
            except Exception as at_err:
                print(f"GOVBID PDF | Airtable update error (non-fatal) | {at_err}")

        try:
            if notify_email:
                send_email(
                    subject=f"Bid Response PDF - {solicitation_number} | ${total:,.2f}",
                    body=(
                        f"Your bid response PDF for {solicitation_number} is ready.\n\n"
                        f"Total Bid: ${total:,.2f}\n"
                        f"Agency: {agency_name}\n\n"
                        f"Review the PDF, sign the signature block, and submit to eMMA "
                        f"at emma.maryland.gov before the deadline.\n\n"
                        f"PDF download: {pdf_url or 'See attachment'}"
                    ),
                    to_email=notify_email,
                    attachment_path=pdf_temp_path,
                )
                print(f"GOVBID PDF | Email sent | {notify_email}")
        except Exception as email_err:
            print(f"GOVBID PDF | Email error (non-fatal) | {email_err}")
